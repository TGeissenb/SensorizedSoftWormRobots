...
def simulate(params):
    ...
    actuation = params["actuation_method"]
    ...
    p.changeDynamics(planeId, -1, lateralFriction=ground_friction)  # set ground plane friction
    ...
    """
    if not os.path.exists(os.path.join("Logs_Comparison_" + actuation)):
        os.makedirs(os.path.join("Logs_Comparison_" + actuation))
    with open(os.path.join("Logs_Comparison_" + actuation, date_time + '_Blob_Log.json'), 'w') as json_file:
        json.dump(parameters, json_file)
    """
    with open(os.path.join("Logs_Bidirectional", date_time + '_Blob_Log.json'), 'w') as json_file:
        json.dump(parameters, json_file)
...
if __name__ == '__main__':
    ...
    for i in range(133):
        ...
        num_obstacles_x = 4
        num_obstacles_y = 15
        spacing_x = random.uniform(3, 8)
        spacing_y = random.uniform(3, 8)
        ...
        params = {"VIDEO_LOGGING": VIDEO_LOGGING,
                  ...
                  "time_step": time_step,
                  "n_steps": n_steps,
                  "actuation_method": actuation}

        #with open(r"C:\Users\tobia\PycharmProjects\SoMo_Simulation\Simulations\LocomotingThreeChannelWorm\Logs_Bidirectional\20240616_113701_Blob_Log.json", 'r') as json_file:
        #    params = json.load(json_file)
        #    params = params["Parameters"]
        #    params["GUI"] = p.GUI

        #simulate(params)
        params["actuation_method"] = "bidirectional"
        simulate(params)
