...
def simulate(params):
    ...
    for i in range(n_steps):
        ...
        puller_torque_1 = torque*stiffness * np.array([0, 1, -np.sin(i*time_step / 10 *2*np.pi), 0])
        puller_torque_2 = torque*stiffness * np.array([0, 1, np.sin(i * time_step / 10 * 2 * np.pi), 0])
    ...
    with open(os.path.join("Logs_Blob", date_time + '_Blob_Log.json'), 'w') as json_file:
...
if __name__ == '__main__':
    ...
    for _ in range(1):
        ...
        base_length = 0.065561175 # random.uniform(0.05, 0.1)
        actuator_length = 0.21543445 # random.uniform(0.10, 0.3)
        base = True # random.choice([True, False])
        if not base:
            base_length = 0.0
        mass = 1.2325674 # random.uniform(0.1, 3)
        center_of_mass = 0.1736816 # random.uniform(0.01, 0.99)  # 0: (0.25) tethered; 1: (0.75) tip; 0.5: balanced
        stiffness = 438.04755 # random.uniform(100, 1000)
        torque = 1.4773878 # random.uniform(0.5, 2)
        ...
        num_obstacles_x = 6
        num_obstacles_y = 15
        obs_height = 0.5419106 # random.uniform(0.5, 5)
        spacing_x = 8.030754 # random.uniform(5, 12)
        spacing_y = spacing_x

        # select physical parameters
        gravity = -9.81
        ground_friction_coefficient = 1.77999 # random.uniform(0.1, 2)
