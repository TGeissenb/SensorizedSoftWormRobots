...
import EPFLcolors
from CurvatureComputation_v2 import closest_point, angle_between


key_frames = 10
live_monitor = False
video_path = r"C:\Users\tobia\Uni\Skripten\Master-Thesis\05 Sensors\Spined Phototransistor Worm\SlowCycleResponse\20240728_CycleResponse_v1.mp4"

df = pd.read_csv(r"C:\Users\tobia\Uni\Skripten\Master-Thesis\05 Sensors\Spined Phototransistor Worm\SlowCycleResponse\20240728_CycleResponse_v1.csv",
                 sep=",")
...
outvid = cv2.VideoWriter(video_path[:-4] + "_Mask.mp4",
                         ...
                         30/key_frames, (int(cap.get(3)), int(cap.get(4))))
...
while(cap.isOpened()):
    ...
    if ret:
        ...
        mask_filtered = cv2.GaussianBlur(mask, (15, 15), 0)
        mask_filtered = cv2.morphologyEx(mask_filtered, cv2.MORPH_CLOSE, np.ones((15, 15), dtype=np.uint8))
        ...
        avg_curv.append(np.mean(k_filtered))
        ...
        axs1.plot(t, np.mean(k_filtered) * np.ones(len(t)), label="$\kappa_{avg}$", color=EPFLcolors.colors[1])
        axs1.set_xlabel("t")
        ...
        ax2.plot(range(len(bend_angle)), bend_angle, color=EPFLcolors.colors[2])
        axs[1].set_xlabel("Frame")
        axs[1].set_ylabel("Average Curvature [-]")
        ax2.set_ylabel("Bending Angle [°]")
...
#axs[1].plot(range(len(avg_curv)), avg_curv, color=EPFLcolors.colors[0])
axs[1].plot(range(len(bend_angle_norm)), bend_angle_norm, color=EPFLcolors.colors[0])
ax2.plot(np.linspace(0, len(bend_angle_norm), len(df["Resistance"])), df["Resistance"], color=EPFLcolors.colors[2])
axs[1].set_xlabel("Frame")
#axs[1].set_ylabel("Average Curvature [-]")
...
import tikzplotlib
tikzplotlib.save(video_path[:-4] + "_Summary.tex")

plt.show()

for i in bend_angle_norm:
    print(i)
